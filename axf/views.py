...
def market(request, cate_id, cid, sortid):
    leftSlider = FoodTypes.objects.all()

    if cid == "0":
        productList = Goods.objects.filter(categoryid=cate_id)
    else:
        productList = Goods.objects.filter(categoryid=cate_id, childcid=cid)
    ...
    group = leftSlider.get(typeid=cate_id)
    childList = []
...
def checkuserid(request):
    ...
    try:
        ...
        return JsonResponse({'data': '该用户名以存在', 'status': 'error'})
    except User.DoesNotExist as e:
